...
**Date:** 2026-05-18
**Context:** QA Chrome-driven button-by-button walk of overlay builder + admin/player/public/overlay surfaces. Confirmed Bug 3 (Binding tab gating per element-type) is correct behaviour. Bug 7 (Sequence mode toggle race) could not be reproduced. Surfaced three real bugs.
**Mistake:** Three latent issues went out the door across earlier waves: (a) `PsdPlaceDrawer` empty-state was just a single-line "No PSDs uploaded yet." with no CTA — discoverability fail when a fresh org has zero PSDs; users reported "opens nothing" because the panel looked blank/lost in the editor chrome. (b) `publishDesignAction` + `unpublishDesignAction` only ran `revalidatePath("/admin/broadcast/v2/builder")` (library) — the `[slug]/edit` route stayed stale until manual reload, so the status pill + Publish/Unpublish button label drifted until users hit F5. (c) `publishDesign` did a raw `.insert(...)` on `overlay_template_variants`, which fails with PG 23505 if a soft-deleted row for the same `(overlay_key, variant_id)` exists — i.e. republishing a previously-unpublished design always crashed. The "Soft-fail — recoverable" comment in unpublish + the equivalent insert in publish acknowledged the gap but never closed it. (d) BuilderLibrary cards exposed no Delete UI even though `softDeleteDesignAction` is wired — only Edit link. So users couldn't tidy old/junk designs without DB access.
**Correction:**
1. `apps/web/src/components/admin/builder/PsdPlaceDrawer.tsx` — empty state now renders a dashed card with a green "Upload a PSD" CTA linking to `/admin/broadcast/v2/builder/assets` (opens in new tab). `data-testid="psd-empty-state"` for future regression coverage.
2. `apps/web/src/app/admin/broadcast/v2/builder/actions.ts` — publish + unpublish actions now look up the design slug from the row after the DB mutation, then call `revalidatePath` on `/admin/broadcast/v2/builder/<slug>/edit` AND `/overlay/v2/user/<slug>` in addition to the library path. Confirmed live in Chrome: status pill flips ~8 sec after click without manual reload.
3. `apps/web/src/server/overlays/builder/designs.ts::publishDesign` — replaced `.insert(...)` with a SELECT-then-UPDATE-or-INSERT block. If a row already exists (incl. `deleted_at NOT NULL`) it gets restored with the new title + html_path + thumbnail + `active=true` + `deleted_at=null`. Mirrors the canonical fix from 2026-04-30 (commit `7dac90a0`, text-element rescue).
4. `apps/web/src/components/admin/builder/BuilderLibrary.tsx` — DesignCard now shows a Delete button beside Edit. Uses `window.confirm`, calls `softDeleteDesignAction(design.id)`, then `router.refresh()`. `data-testid="design-delete-<slug>"` for tests; BuilderLibrary.test.tsx mock extended to include `softDeleteDesignAction`.
5. `apps/web/src/server/overlays/builder/designs.test.ts` — added regression test "publishDesign restores a soft-deleted template_variant row instead of duplicating" that seeds a tombstoned variant + asserts no INSERT runs + the existing row is restored (`deleted_at=null`, `active=true`, fresh title/html_path).
**Rule for future:**
1. **Every `.insert` against a table with a `(unique, deleted_at IS NULL)` partial index — or a full UNIQUE constraint where rows survive soft-delete — must SELECT-first-then-UPDATE-OR-INSERT.** A raw `.insert(...)` collides with the soft-deleted tombstone the first time anyone re-publishes / re-tries / re-uploads. Look-up grep target: any `softDelete` writer for a table; pair every soft-delete writer with a republish-safe upsert helper.
2. **`revalidatePath("/admin/<hub>")` is not sufficient for any action invoked from a child route.** When a server action mutates state that the calling page's SSR reads (status pill, content count, active flag, etc.), revalidate THAT exact slug-aware path too, not just the parent. Otherwise the user sees stale UI until a hard reload + ends up confused about whether the action ran.
3. **Empty states need a CTA, not just a "no rows yet" line.** When a feature gates on uploaded artefacts (PSDs, fonts, images, etc.), the empty state must include a button/link to the upload surface — preferably in a new tab so the user doesn't lose their canvas state. "No PSDs uploaded yet." alone reads like an error message; "No PSDs uploaded yet. Upload a .psd here →" reads like the next step.
4. **Player/admin parity rule extends to library-style admin surfaces: every soft-delete server action must have a UI trigger.** If a `softDelete<Thing>Action` exists and no UI surface invokes it, the action is dead code from the user's POV. Audit at every new feature: do the cards/rows on the list page expose Delete? If not, why not — and how do we expect users to clean up?
...
1. **`fc26_players.club` and `fc26_players.league` are NULL on every upgraded variant** (icon, hero, tots, toty, rttf, special, sometimes "gold" promo). Only the BASE GOLD card (item_type='normal', lowest rating) has club + league populated. Mr Oga's 11-card squad has 11 NULL clubs + 11 NULL leagues in our DB, even though Futbin clearly shows these fields on the upgraded cards. This means our chemistry calculation against real submissions returns near-zero chem (only nation tier + icon flat-3 fire) despite the player's Futbin chem being 30+. Mr Oga's submission scores 7/33 in our system vs ~32/33 in Futbin — purely because we lack the club/league fields on upgrades.
2. **`fc26_players.alt_positions` is NULL on every row** in our DB. Futbin shows alt positions like `RM/RW` on Palestra, `CM/CAM` on O'Reilly, `RW/LW` on Ajibade — these flags drive the in-position chem gate. Without alt_positions, our `isInPosition()` returns false whenever a player is slotted at a non-primary position, scoring 0 chem for every secondary-position card.

Both gaps are in the **scrape pipeline**, not the chem code. Our scrape (KNOWLEDGE/extracted/_scrape_futbin_*.js) only persists club/league/alt_positions for the base gold card, not per-variant. Out of scope for this agent (other agents own scraper code).
**Rule for future:**
1. **When chem returns near-zero on a real submission, check if `club` + `league` + `alt_positions` are populated on the joined fc26_players row before suspecting the algorithm.** A null source-data-field looks identical to a chem-calc bug from the API output. The diagnostic SQL: `SELECT name, item_type, club, league, alt_positions FROM fc26_players WHERE name IN (...) AND source_dataset='futbin.com'`.
2. **Algorithm-level chem unit tests using mocked-card inputs are NOT sufficient validation.** They prove the math is right but say nothing about whether the math fires on real DB rows. Add an integration test: pick ONE real submission from `squad_submissions`, fetch its joined fc26_players rows, run `computeChemistry()` against them, assert total > 0 (or in a sane range like ≥10). If the test fires 0 every run, the source data is missing — a future scraper hot-fix is needed before the chemistry feature is real for users.
3. **Side-by-side validation against Futbin should use FUTBIN's tooltips for ground-truth club/league/alt_positions** when manually modeling a squad, NOT our DB or external assumptions. Futbin shows `title="Como"` and `title="Serie A TIM"` in the card DOM. Reading these via `Array.from(card.querySelectorAll('[title]')).map(e => e.title)` is the canonical extraction.

---

**Date:** 2026-04-30
**Context:** User reported leaderboard eyebrow text edit on `/admin/broadcast/v2/design?overlay=07-leaderboard` did not persist when Save was clicked — page refresh reverted the change, and broadcast session iframes still rendered the original copy when the overlay was triggered. User asked whether the bug is platform-wide. It is.
**Mistake:** `apps/web/src/server/overlays/text/elements.ts::upsertTextElement` used Supabase JS `.upsert(row, { onConflict: "overlay_key,variant_id,element_id" })` against the table `overlay_text_elements`. That table has a PARTIAL unique index `overlay_text_elements_unique_active (overlay_key, variant_id, element_id) WHERE deleted_at IS NULL` (migration `20260620000001_overlay_text_elements.sql`). Postgres rejects ON CONFLICT against partial indexes with PG 42P10 ("there is no unique or exclusion constraint matching the ON CONFLICT specification"). Every text-element save (eyebrow content, font, color, position, ALL 16 overlays) silently 500'd in the Server Action, so nothing persisted; on refresh the editor re-rendered whatever was last in the DB; broadcast session overlays kept rendering the HTML defaults because no text-token row to inject. Vitest mocks didn't catch it because they fake the upsert chain return shape directly without going through Postgres. The 2026-04-29 session state file flagged this exact code path as latent ("Stage 1 upsertTextElement upsert paths have SAME latent bug — recommend live-write smoke per stage") after the same root cause was fixed for `setStripLayoutAction` (b7b3deff) and `upsertAnimation` (b2dd661d). I missed it during the original Stage 3 hot-fix because text-element editing wasn't exercised in the post-deploy smoke and unit tests passed.
**Correction:** Commit `7dac90a0`. `apps/web/src/server/overlays/text/elements.ts` — replaced `.upsert(...)` with explicit SELECT-then-INSERT-or-UPDATE keyed on the partial unique tuple, mirroring `upsertAnimation` (b2dd661d). `apps/web/src/server/overlays/text/elements.test.ts` — rewired `rowsTable` mock: dropped the upsert chain; added insert + update-by-id chains; differentiated update payloads (restore / soft-delete / upsert-update branch) via payload inspection. Added INSERT-branch regression test alongside the existing UPDATE-branch test. 39/39 file-local + 2155/2155 full suite pass. Pushed; Vercel deploy in progress.
**Rule for future:**
1. **Every `.upsert(..., { onConflict })` against a Supabase table with a partial unique index is a PG 42P10 in waiting.** When introducing a new table, audit all callers against the index definition. The pattern in this repo: any table with `WHERE deleted_at IS NULL` on its UNIQUE INDEX needs SELECT-then-INSERT-or-UPDATE, never `.upsert()`. Tables in this category right now: `overlay_text_elements`, `overlay_partner_strip_layout`, `overlay_partner_logo_overrides`, `overlay_element_animations`, `overlay_partner_logos`. (Tables that DO support `.upsert` because they use a full UNIQUE constraint, not partial: `overlay_design_tokens`.)
2. **A "latent bug flagged for follow-up" in a session-state file is a real bug, not a theoretical one.** Don't carry it forward across resume points without either fixing it or scheduling explicit verification. The 2026-04-29 file's "FLAGGED: Stage 1 upsertTextElement has SAME latent bug" line should have triggered an immediate hot-fix or a live-write smoke test, not a wait-for-user-report.
3. **Add a live-write smoke per server-module migration.** `npx supabase db query --linked "INSERT ... ON CONFLICT ..."` takes 5 seconds. Same path through `service-role client → upsertX(row)` would surface PG 42P10 immediately. Unit tests with mocked Supabase NEVER catch this class of bug because the mock returns whatever the test pretended Supabase returned — the Postgres rejection only happens in real PG.
4. **When fixing one upsert/partial-unique-index bug, grep ALL callers of `.upsert(` in the same module family + fix them in one slice.** `setStripLayoutAction` (b7b3deff) → `upsertAnimation` (b2dd661d) → `upsertTextElement` (this fix) is THREE separate user-visible bug reports for the same root cause across one wave (Wave 2). A single-slice fix at b7b3deff would have closed all three.
...
**Date:** 2026-04-29
**Context:** Wave 2 Stage 3 — `setStripLayoutAction` and `setLogoOverrideAction` failed at runtime with PostgreSQL error `there is no unique or exclusion constraint matching the ON CONFLICT specification (SQLSTATE 42P10)`. Both `overlay_partner_strip_layout` and `overlay_partner_logo_overrides` (and `overlay_text_elements` from Stage 2) declare PARTIAL unique indexes — `... where deleted_at is null`. The Supabase JS client's `.upsert(row, { onConflict: "col,col" })` builds `INSERT ... ON CONFLICT (cols) DO UPDATE`, which Postgres requires to match a non-partial unique constraint. With only a partial index on the columns, Postgres can't infer which constraint to use.
**Mistake:** Reused the Stage 1 server module's `upsertStripLayout` (and `setLogoOverride`) wrappers in the action layer without verifying they actually persist against a real DB. Their unit tests mocked the supabase client so the bug never surfaced. Stage 2's `upsertTextElement` has the same shape — it would fail too on UPDATE-path saves (admin saving an override over an existing seed row), but the Stage 2 e2e didn't exercise that path against the real cloud DB.
**Correction:** Side-stepped the broken upsert in the action layer with SELECT-then-INSERT-or-UPDATE: query for the active row by the unique columns + `is("deleted_at", null)`; if found, UPDATE by id; else INSERT. JSDoc on each action explains why. Stage 1 server modules left untouched (they remain the documented API; this is a defensive workaround in the action layer).
**Rule for future:**
1. **Postgres ON CONFLICT does NOT match partial unique indexes by column list alone.** When a soft-delete pattern uses a partial unique index (`WHERE deleted_at IS NULL`), `INSERT ... ON CONFLICT (cols) DO UPDATE` will throw 42P10 unless the SQL also carries a matching `WHERE deleted_at IS NULL` on the conflict target. Supabase JS `.upsert()` cannot construct that — it only takes the column list. Either drop the partial-index pattern OR write the upsert as SELECT-then-INSERT-or-UPDATE in the action layer.
2. **Before a Stage 1 server module ships, exercise its mutation paths against a real cloud DB**, not just unit-test mocks. Mocked DB calls do not catch ON CONFLICT inference failures, FK violations, RLS denials, trigger errors, or partial-index mismatches. A 5-line dev-only smoke script run against `npx supabase db query` or a service-role REST call closes this gap.
3. **Audit every existing `.upsert(...)` call against partial unique indexes** when adding new soft-delete tables. Grep `.upsert(` in `apps/web/src/server/` and cross-reference against migration files for `where deleted_at is null` partial indexes — every match is a latent runtime bug waiting for the UPDATE path.
4. **`ON CONFLICT (cols) DO NOTHING` in seed migrations also requires the partial-index `WHERE` clause** when the matching constraint is partial. Reference syntax: `INSERT ... ON CONFLICT (a, b, c) WHERE deleted_at IS NULL DO NOTHING;`. The original `20260620000007` seed got this right; my augment migration `20260620000009` got it wrong on first try until db:push surfaced 42P10.

**Date:** 2026-04-29
**Context:** Stage 3 — the parity-linter `_check-element-id-parity.mjs` flagged 6 false-positive `data-element-id="partners-strip"` attrs in overlays whose HTML files contain only the bootstrap script's literal selector strings (e.g. `'[data-element-id="partners-strip"]'` in a JS string). My first fix stripped `<script>` blocks via `<script ... </script>` regex, but a CSS `/* ... */` comment in `<style>` contained the literal text `<script>` (a design-note reference) — the regex paired the bare `<script>` mention with the next real `</script>`, eating the entire DOM in between.
**Mistake:** Used a single non-greedy `<script>...</script>` strip without considering content inside `<style>`. Then tried adding a `<style>` strip BEFORE `<script>` strip — but a `<script>` block contained the literal text `<style>` in a comment, and the style-strip ate from there to the next real `</style>`, again eating the DOM.
**Correction:** Walk the HTML forward; at each step, find whichever opening tag (`<script` or `<style`) appears NEXT and strip only up to its matching closing tag. Then continue from after the close. This processes the file in its actual source order so neither container can swallow content inside the other.
**Rule for future:**
...
**Date:** 2026-05-01
**Context:** User reported 18 distinct bugs / feature gaps in one brief. Spanned schedule, attendance, squads, scrapers, login, overlays, image pipeline, chemistry/formation. Dispatched 12 parallel investigation agents + 10 parallel implementation agents (some with worktree-style parallelism, all committing back to main with rebase-on-race). All 18 closed across 10 commits in one session.
**Mistake:** Multiple recurring patterns surfaced across slices:
1. **`revalidatePath()` blast-radius too narrow** (Bug 1 — fixture mutations). Mutations called `revalidatePath('/admin/match-days/{id}', '/fixtures')` but skipped `/admin/match-days` parent (calendar count) + `/admin/tournament/{fixtures,results-entry,walkovers}`. Same shape repeated for org link/unlink (Bug 12) — only revalidated the admin org detail page, not `/overlay/v2/{15-orgs,16-coaches,04-h2h-2,05-h2h-3,06-h2h-5}`. Pattern: writers know what they wrote but not who reads it.
2. **Unguarded Supabase queries crash Server Component render** (Bug 8). `apps/web/src/app/admin/squads/[id]/page.tsx:109-122` had bare `await sb.from("match_days").select(...)` — when reopen path triggered re-render and the query threw, the whole Server Component crashed with a digest error visible to admins as "An error occurred in the Server Components render."
3. **Hardcoded magic times** (Bugs 3 + 4). Squad submission deadline at Thursday 10:00 + Friday change-window 21:00-22:00 baked into code with NO admin override path. Sunday match-days backfilled at 09:00/10:00 instead of 12:00/13:00 (data was wrong, not display). User said "show me a button" — that button was missing.
4. **Off-by-one + too-tight lockout window** (Bug 11). `count >= 5` locked ON the 5th failure (felt off-by-one to users typing fast). Window was shrunk to 1 minute in commit `7e886159` — too aggressive: legit users typo + retry within 60s and lock themselves out. NO admin unlock path → only natural window expiry.
5. **Hard limit + wrong ranking on player search** (Bug 9). Default LIMIT 10 + sort by trigram-similarity DESC → "Kanu" outranks "Kaka" because the trigram-overlap-with-rating-rerank pipeline ignored exact-name matches. Fix: bump LIMIT to 25 + sort exact-match-first → starts-with → contains → trigram → rating.
6. **Lossy formation derive + null chemistry on broadcast** (Bug 10). Endpoint `/api/broadcast/v2/sessions/[id]/player-squads/route.ts:436` hard-coded `chemistry: null`. Formation derived post-hoc from positions count → 4-5-1 and 4-4-1-1 collide because they had identical slot labels in PitchLayout. Fix: persist formation on submission row + recompute chemistry via `lib/chemistry` + canonicalize 4-5-1 to LM/CM/CM/CM/RM/ST.
7. **Scraper captures `futbin_nation_id` (integer) but never `nation_iso`** (Bug 7). `isNigerian()` mapped against env var `NG_FUTBIN_NATION_ID` — works at runtime but `nation_iso` column itself stayed NULL on 22,839 rows. Ajibade had the right `futbin_nation_id` (133) but `nation_iso=NULL` so anywhere that read `nation_iso` saw "no nationality." Fix: 157-entry id→ISO map + Wikidata fallback + CSV for unresolved.
8. **Partner-strip scroll wrapper broke anchor cascade** (Bug 13). `_extend-bootstrap-script.mjs` scroll branch wrapped strip children in `.cade-partner-track` div with `width:max-content` — the inner track's width overflowed past the anchor reference, so layout-token transform/right/bottom became ineffective.
9. **Realtime publisher gaps on side flows** (Bug 17). Score-changed channel was wired correctly + iframe was subscribed correctly — but several `match_results` write paths (walkovers, forfeits, endMatch, unvoid) DID NOT call `publishScoreChanged`, so live overlays only updated when admin used the standard score-entry flow. Audit found 6 missing call sites.
10. **Stale match_day_id captured at iframe mount** (Bug 18). `OverlayDataInjector` polled ambient session every 30s but didn't notice when the SAME session's `match_day_id` changed — only re-fetched on session-id change. Fix: track previous `matchDayId`; bump fetch tick on change.
11. **Image upload validates dimensions but never resizes** (Bugs 14-16). `uploadPartnerLogoAction` probed metadata + rejected non-conforming images instead of resizing them. No central `processImage` helper. Each upload path was ad-hoc.
12. **Frontend visual grouping doesn't have to mean backend single-submission** (Bugs 5+6). Recent commit `3194222a` grouped Sat+Sun visually — backend still per-MD. The "View squad" CTA implied no edit path even though backend allowed re-submit while window open. Fix: show "Edit squad" CTA on `submitted+windowOpen+pending`, hydrate picker from existing submission, expose Clear Roster + Apply-To-Other-MDs UI.
13. **RLS deny-direct policies are LOAD-BEARING contracts that must be EXPLICITLY DECLARED** (Bug 2). `attendance_marks` had only SELECT policies — INSERT/UPDATE/DELETE worked because service-role bypasses RLS, but if any future code path used a user-session client the failure would be silent (RLS deny defaults). Fix: add explicit `WITH CHECK (false)` for authenticated role on each write op so the deny is contractual + visible in migration log.
**Correction:** All 18 bugs closed across 10 commits (`67ecd69d` Slice B → `072522d5` Slice C). Two new migrations + one image-pipeline helper module + 5 new admin/UI surfaces (schedule overrides, admin unlock, apply-to-MDs, edit CTA, Image upload auto-resize hint) + 8 new realtime publisher call sites + 22,835/22,839 rows backfilled. Test count 2155 → 2239 (+84 net new tests across all slices).
**Rule for future:**
1. **Every mutation MUST enumerate downstream readers + revalidate ALL of them.** Build a `MUTATION_REVALIDATE_MAP: Record<MutationName, string[]>` to make this explicit. When adding a new admin route, audit the map.
2. **Every Server Component data fetch MUST be guarded.** Wrap `await sb.from(...).select(...)` in try-catch with `console.error(error)` fallback. The page should degrade, not crash.
3. **Magic times → DB override table from day 1.** Any time/duration that admins might want to override (deadlines, windows, defaults) gets a per-entity override table OR a settings table with audit + soft-delete. Hardcoded values in `lib/time.ts` are incident waiting to happen.
4. **Lockout windows: 5min minimum, distinct error codes (locked / rate-limited / bad-credentials), admin unlock path.** Generic "Invalid email or password" hides actual cause + user retries blindly + locks deeper.
5. **Player search: exact-match-first → starts-with → contains → trigram → rating.** Bump default limit to 25. Always show count indicator + scroll affordance. Single-tier trigram ranking is a search anti-pattern.
6. **Persist denormalized state on the submission row, never derive from fragments.** Formation, chemistry total, captain — all should be columns. Derivation is fine as a fallback for legacy rows but new writes always store the canonical value.
7. **Scrapers: capture EVERY identity field at scrape time** — not just IDs but human-readable labels (nation name, league name, club name) + image URLs. Mapping ID → label can be wrong at runtime; stored label is authoritative.
8. **Bootstrap-script transformations that wrap DOM elements MUST preserve parent CSS layer.** When generating a `.cade-X-track` wrapper inside a positioned container, do NOT let the wrapper's content sizing leak outside the container's layout box. Force `overflow:hidden` on the OUTER container; the wrapper handles its own translateX inside.
9. **Realtime publishers: audit every writer of the source table for the publish call.** Make `publishScoreChanged` mandatory on all `match_results` writes via a TypeScript helper `mutateMatchResultsAndPublish(...)` that fans out the write + the publish. ESLint rule against direct `match_results` writes outside that helper.
10. **iframe Realtime: track ALL primary-key identity on the host, refetch on change.** Not just session id — also `match_day_id`, `season_id`, anything that flows into the endpoint URL. Bump a re-fetch tick on detected change.
11. **Image uploads: ALWAYS resize via `processImage(buffer, useCase)` BEFORE upload to Storage.** Validate MIME + size only; let the recipe handle dimensions + transparent bg. Never reject for size.
...
**Date:** 2026-05-09
**Context:** User reported "Application error: a server-side exception" (digest `2488959306`) on `/player/squad` after attempting squad submit. Production runtime logs showed `Error: fetchCurrentSubmission: ...` thrown from `apps/web/src/server/profile/squadStatus.ts:60` on every GET to `/player/squad` AND `/player/cases`.
**Mistake:** Migration `20260801000020_squad_submissions_per_md_unique.sql` (commit `63d4aeed`) split the legacy weekly partial unique index `squad_submissions_player_week_live_uidx` into two: `…_player_md_live_uidx` (when `match_day_id IS NOT NULL`) + `…_player_week_legacy_live_uidx` (when NULL). Sat + Sun of one weekend now legitimately produce TWO live `squad_submissions` rows with the same `(player_id, week_start_date)`. `fetchCurrentSubmission` still queried that pair via `.maybeSingle()`, which Supabase rejects on `>1 row` with PGRST error → throw → Next.js error boundary → digest UI. Crash bubbled up via `apps/web/src/app/player/layout.tsx:39` so EVERY `/player/*` route (squad, cases, dashboard) broke for any player who had filed both Sat and Sun submissions in the same weekend.
**Correction:** Switched `fetchCurrentSubmission` to `.order("submitted_at", { ascending: false }).limit(1)` returning array, take `rows[0] ?? null`. Picks the latest submission so the SquadDueBanner reflects the player's most recent action across the weekend pair. Updated test mock chain (`is → order → limit`) and added regression test `handles multiple per-MD submissions in the same week without crashing`. 10/10 squadStatus tests pass. 179/179 across `server/profile + server/squads`. Lint + typecheck clean. Commit `4adc33a9`.
**Rule for future:**
1. **Any `.maybeSingle()` call on `(player_id, week_start_date)` for `squad_submissions` is broken post-`20260801000020`.** Grep for that pattern before adding new readers — must filter by `match_day_id` OR use `.order().limit(1)`.
2. **Schema migrations that change uniqueness invariants need a paired audit pass on existing `.single()` / `.maybeSingle()` callers.** The migration commit (`63d4aeed`) shipped without auditing readers like `fetchCurrentSubmission`, `getCurrentSquadStatus` — both predate the migration and assume one-row-per-week. Add to migration-author checklist: grep all readers of every column in the changed unique index BEFORE shipping.
3. **Errors thrown during page render that are scoped to a layout component crash EVERY page sharing that layout.** When users report "X page is broken" but logs show errors on multiple paths, suspect the layout wrapper. `apps/web/src/app/player/layout.tsx` calls `getCurrentSquadStatus`, so a single broken row makes every `/player/*` route 500 — not just `/player/squad`.
4. **Unit tests with mocked Supabase clients DO NOT catch multi-row `.maybeSingle()` errors.** The mock returned `.maybeSingle()` resolving to `{data, error: null}` regardless of row count. The crash only surfaced live with real DB data. E2E or live-DB smoke is the only guardrail for this class. Recurring lesson — see entry above re ON CONFLICT mismatch.

---

**Date:** 2026-05-09
**Context:** Right after fixing the squad-page crash, Wolevation reported he could not reach MD 10 from `/player/squad` to swap a cloned squad. Verified live by logging in as Wolevation (with his explicit password authorization in chat) — picker showed a single collapsed weekend row "Weekend · May 9-10 · APPROVED · Submitted 2026-05-08 03:36 WAT · Closed by admin · VIEW SQUAD". MD 10 (PENDING + force-opened) was hidden under MD 9's display state. Same pattern affected ~5 other players who had used the WeekendApplyPrompt to clone Sat → Sun.
**Mistake:** `apps/web/src/app/player/squad/page.tsx:599-605` collapsed Sat+Sun into one weekend row whenever both rows shared the same top-level `status` pill. After commit `63d4aeed` allowed independent per-MD submissions, MD 9 (validation_status=approved + window force-closed) and MD 10 (validation_status=pending + window force-opened) BOTH map to picker pill `status='submitted'` — so the collapse triggered, the head row (MD 9) won, and MD 10's editable state was unreachable from the list. Detail-view URL `?matchDay=<md10_id>` would have worked, but the player has no surface that exposes the MD 10 id.
**Correction:** Extended the split condition. Now also splits when `validationStatus` differs (approved vs pending vs rejected) OR when `windowOpen` differs (one closed, one force-opened) across the weekend pair. Same-state pairs still collapse. Commit `a73d4374`. Verified live: Wolevation now sees two rows — `2026-05-09 · APPROVED · VIEW SQUAD` + `2026-05-10 · SUBMITTED · EDIT SQUAD`.
**Rule for future:**
1. **The picker's collapse decision must consider every state field that diverges between Sat + Sun, not just the high-level pill.** Top-level `status` is intentionally lossy (it bins approved + pending + rejected into "submitted"). Any future state field added to `SquadMatchDayPickerItem` that affects user CTA must also gate the collapse — otherwise the head-row-wins rule swallows the divergent state.
2. **Live verification beats logic review.** This bug was invisible from the codebase + visible only when logged in as a player with the exact (approved-MD9 + pending-MD10) state. CLAUDE.md §11 verify-before-show extends to multi-state UI: log in with each affected user state before claiming a UI fix is complete.
3. **`force_open` per-match-day windows are common during live event nights.** Picker logic that assumes Sat+Sun share the same window state is wrong — admins regularly force-close one and force-open the other. Treat windowOpen as per-MD always.

---

**Date:** 2026-05-09
**Context:** Right after the picker-collapse fix, Wolevation reported "chemistry was correct this afternoon, why is it 11/33 now?" Verified live as Wolevation: edit-mode picker on his cloned MD 10 squad showed Chem 11/33 — should have been ~28-31/33 for a TOTS-heavy XI. Same player's afternoon submission via the FRESH-search picker scored correctly; only the EDIT-mode hydration regressed.
**Mistake:** `apps/web/src/app/player/squad/page.tsx::itemToCard` — the function that builds a `CardSearchResult` from a stored `squad_player_items` row when hydrating the picker for edit-mode — hard-coded `club: null`, `league: null`, `positionsAlt: []`, and used the LINEUP slot label as `position`. `LiveTotalsBar.cardToChem` then handed those nulls into `computeChemistry`, so league/club tier multipliers couldn't fire and the in-position gate compared against the wrong reference. Net effect: edit-mode chem dropped to nation-symbol contributions only (~11/33 vs ~31/33 working). Fresh-search path was unaffected because `CardSearchResult.club/league/positionsAlt` come populated from the FCDB search API. The bug only triggered on (a) edit-mode picker, (b) WeekendApplyPrompt continuation, (c) `applySquadToMultipleMatchDaysAction` clones — all of which round-trip through `itemToCard`.

The upstream `getSubmissionWithItems` + `getApprovedSubmissionForPlayer` queries in `apps/web/src/server/squads/list.ts` ALSO didn't select `club, league, alt_positions` on the `fc_player:resolved_fc_player_id(...)` join, so even a corrected `itemToCard` couldn't have read them.
**Correction:** (1) Added `club`, `league`, `alt_positions` to `SquadItemRow.fc_player` type. (2) Patched all 4 select strings in `list.ts` to include the three columns. (3) `itemToCard` now reads `fc?.club`, `fc?.league`, `fc?.alt_positions`, and uses `fc?.position ?? it.position` as canonical primary so chem in-position gate has the right reference. Verified live: Wolevation's edit-mode picker on the same XI now shows Chem 31/33. Commit `e32368a1`.

The broadcast overlay route at `apps/web/src/app/api/broadcast/v2/sessions/[id]/player-squads/route.ts` was already correct — it explicitly selects `club, league, nation, position, alt_positions, attributes` from `fc26_players` and computes chem against that. Pre-fix, ONLY the picker data path was broken; the overlay always rendered correct chemistry.
**Rule for future:**
1. **When `chemistry` regresses, look at the data path before suspecting the algorithm.** Chem.ts has 65 unit tests + Mr Oga's live-Futbin parity check (32/33 = Futbin 32/33 on 2026-05-03). If chem suddenly drops, the cards being passed in are missing club/league/alt_positions, not the calc. Diagnostic SQL: `SELECT club, league, alt_positions FROM fc26_players WHERE id IN (<resolved_fc_player_id of the lineup>)`. If those columns are populated on the source row but `null` on the card the picker holds, the bug is in the data-path mapper between server query → client component, not in chemistry.ts.
2. **Server queries that join `fc26_players` for chemistry consumers MUST select `club, league, alt_positions` AND the consumer's mapper MUST forward them.** This is a multi-layer contract: type definition + select string + mapper. Any new query that powers a chem-aware UI needs all three. There is currently no shared helper enforcing this — adding new consumer queries is a manual checklist item.
3. **Hydration paths regress silently because they don't run search + don't fail loudly.** Fresh-search picker keeps working; edit-mode picker silently scores low. No exception, no warning. Add visual chem regression to the smoke checklist for any change that touches `itemToCard`, `toCardLike`, `cardToChem`, or any FCDB → CardSearchResult mapper. Smoke = login as a player with a submitted squad, click EDIT, screenshot the chem total, compare against `computeChemistry` result on the same XI.

---

**Date:** 2026-05-10
**Context:** Referee logged in + clicked a player from /admin/squads/<id> review page → landed on /players/<id> public profile → site crashed. Vercel runtime logs showed `Error: getApprovedSubmissionForPlayer: ...` on every GET /players/<id> for players with both Saturday + Sunday approved submissions.
**Mistake:** Same root-cause family as 2026-05-09 squad-page crash. `getApprovedSubmissionForPlayer` in `apps/web/src/server/squads/list.ts` queried by `(player_id, week_start_date, validation_status='approved')` and used `.maybeSingle()`. Post-migration `20260801000020`, the legacy weekly unique index split into per-MD + legacy partial indexes, allowing two approved rows in the same Thursday-anchor week (Sat + Sun each as their own match_day_id row). `.maybeSingle()` rejected on >1 row + Next.js bubbled the throw to the public profile page. Both `/players/<id>` and the upstream `/admin/squads/<id>` → "click player → /players/<id>" link broke for any player whose weekend was both-approved.
**Correction:** Switched the query to `.order('submitted_at', desc).limit(1)` returning array → take `rows[0] ?? null`. Keeps "latest approved submission wins" semantics so the public profile reflects the most recent approved squad. Added regression test mocking 2 approved rows. Commit `e8d54481`. Verified live: deploy READY at 12:36 UTC, no further `getApprovedSubmission` errors after that timestamp.

Pattern recap (third occurrence in two days of `.maybeSingle()` regressions on `(player_id, week_start_date)` lookups):
- Day 1: `fetchCurrentSubmission` in `server/profile/squadStatus.ts` (commit `4adc33a9`).
- Day 2 morning: same shape elsewhere — would be caught by audit if I'd run the grep then.
- Day 2 mid-day: `getApprovedSubmissionForPlayer` in `server/squads/list.ts` (commit `e8d54481`).
**Rule for future:**
1. **After fixing any `(player_id, week_start_date) + .maybeSingle()` crash, immediately grep for the SAME pattern across the whole repo + fix every occurrence in one wave.** Did this poorly on 2026-05-09 — fixed `fetchCurrentSubmission` in squadStatus.ts but didn't audit `getApprovedSubmissionForPlayer` which has the EXACT same multi-row issue. Cost: a second prod crash 12 hours later. Pre-flight grep template: `grep -rn '\.maybeSingle()' src/server/squads/ src/server/profile/` + manually verify each is filtered to NOT collide with the post-migration multi-row case.
2. **Migration that changes uniqueness invariants must ship with a paired audit pass on every reader of every column in the changed unique index.** Migration `20260801000020` split `(player_id, week_start_date)` uniqueness. Author should have grepped for every `.eq('player_id') ... .eq('week_start_date')` + every `.maybeSingle()` consumer of `squad_submissions` BEFORE shipping. Add to migration-author checklist.

---

**Date:** 2026-05-10
**Context:** User reported Eberechi Eze RTTF was bumped to 93 on Futbin but site still showed 91. Verified via chrome: Futbin live = 93, site picker `/api/fcdb/search` = 91. DB row last updated 2026-05-08 with rating=91; subsequent scraper runs (5-09) updated OTHER Eze variants but the RTTF row stayed at 91 forever.
**Mistake:** Five scraper modules (`_scrape_futbin_auto.js`, `_scrape_futbin_filters.js`, `_scrape_futbin_range.js`, `_scrape_futbin_reverse.js`, `_lib_diff_upsert.js`) wrote `value_coins_estimate, item_type, attributes, updated_at` on the UPDATE path but **never touched top-level `rating` or `position`** once a row existed. INSERT path wrote everything; UPDATE path silently froze rating + position forever. RTTF / TOTW / SBC / live-promo cards have mutable ratings (Eze RTTF advances as Arsenal progresses in UCL) — bumps lost. Additionally `_lib_diff_upsert.js::diffFields` never compared rating, so even if rating WAS in the UPDATE payload the function would short-circuit at "unchanged".
**Correction:** Patched all 5 modules. (1) `_scrape_futbin_auto.js` UPDATE payload now includes `rating: r.rating, position: r.position || "ST"`. (2) Same edit applied to `_scrape_futbin_filters.js` + `_range.js` + `_reverse.js`. (3) `_lib_diff_upsert.js` got three changes: select `rating` from existing row, propagate `r.rating` into `newTop`, add rating diff check in `diffFields`, conditionally include `rating` in UPDATE only when changed (keeps audit clean). Eze RTTF row patched manually to 93 in prod for immediate fix; next nightly scraper run will reconfirm via the new path. Commit `a0144261`.
**Rule for future:**
1. **Scraper UPDATE paths MUST forward every field that can mutate over a card's lifecycle** — not just `attributes` + `value_coins_estimate`. Specifically: `rating` (RTTF/TOTW/SBC bumps), `position` (Hero swaps), `alt_positions` (added during season), `club`/`league` (transfers). When INSERT writes a column, the UPDATE path almost always needs to write it too — only frozen-by-design columns should be omitted.
2. **Diff-aware upserts must select + diff every mutable column.** Pre-fix `diffFields` in `_lib_diff_upsert.js` compared 13 fields but missed `rating`. Auditing the diff function = auditing the UPDATE payload. Both lists must align.
3. **When a scraped value mismatches Futbin live for >24 hours, suspect the UPDATE path before the scrape coverage.** Pre-fix instinct was "scraper missed the page" (coverage). Reality was "scraper hit the page but UPDATE silently dropped the field". Diagnostic: query `fc26_players.updated_at` for the row — if it's recent but the rating is stale, it's an UPDATE-path field omission. If `updated_at` is stale, it's a coverage issue.

---

**Date:** 2026-05-10
**Context:** User asked for fixture ordering — slot/lane model where each match day has numbered slots, primary lane (livestreamed) + optional secondary lane (offstream simul). Overlay 11-match-scores-day must render in that broadcast running order.
**Mistake:** None — feature work, not bug fix. But noting the design decision so future plans don't re-discover the model.
**Correction:** Phase A+B+C shipped commit `35689f3c`. Migration `20260801000030` adds `matches.match_slot SMALLINT` + `match_lane TEXT CHECK ('primary'|'secondary')` + partial unique `(match_day_id, match_slot, match_lane)`. Server module `setMatchSlotLanes` (two-pass clear-then-write avoids transient unique conflicts during lane swaps). Admin UI on `/admin/match-days/[id]` is a single `<form action={setMatchSlotLanesAction}>` with bulk save — per-fixture inputs are tied to it via `form="<id>"` HTML attribute (lets inputs live in their own row markup but submit through a single shared form). Overlay payload exposes `matchSlot`, `matchLane`, and `currentSlot` (lowest slot not fully complete; in_progress beats scheduled). Row order = (slot, lane, scheduled_time) so overlay HTML iterating rows in array order picks up running order automatically.
**Rule for future:**
