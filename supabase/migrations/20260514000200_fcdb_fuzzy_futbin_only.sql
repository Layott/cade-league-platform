-- 2026-04-23 — Restrict fc26_players_fuzzy RPC to the live Futbin slice.
--
-- Per CLAUDE.md §Dormant datasets, all fcdb runtime lookups MUST filter
-- `source_dataset='futbin.com' AND deleted_at IS NULL`. The RPC (used
-- by ref-review + squad-picker typeahead) was missing the source filter
-- so it could return Kaggle/fut.gg rows that carry no prices/images.
--
-- Rewrite is identical to migration 20260506000005 except the WHERE
-- clause adds `p.source_dataset = 'futbin.com'`.
