 ...
 *
 * Pick a fixture (only those without a result), pick the winner, and a
...
export function TriggerWalkoverForm({
  ...
  const [matchId, setMatchId] = useState<string>(candidates[0]?.matchId ?? "");
  ...
  if (candidates.length === 0) {
    return (
      <div className="rounded-sm border border-dashed border-[var(--ink-4)] bg-[var(--ink-2)]/40 p-6 text-center text-[12px] text-[var(--chalk-3)]">
        No fixtures available for direct walkover — every match either has a
        result already or there are no scheduled fixtures.
      </div>
    );
  }
          ...
          >
            {candidates.map((c) => (
              <option key={c.matchId} value={c.matchId}>
                {c.matchDayLabel} · {c.homeName} vs {c.awayName}
              </option>
            ))}
          </select>
        </label>
