...
async function resolveAdmin() {
  ...
  if (!auth.user) redirect("/login?next=/admin/broadcast/v2/stingers");
  const { data: pub } = await userClient
    .from("users")
    .select("id")
    .eq("supabase_auth_id", auth.user.id)
    .maybeSingle();
  if (!pub) redirect("/login?next=/admin/broadcast/v2/stingers");
          ...
          <ul className="space-y-1 text-sm">
            {recent.map((r) => (
              <li key={r.id} className="flex justify-between rounded-sm border border-[var(--ink-4)] bg-[var(--ink-2)] px-3 py-2">
                ...
                <Link
                  href={`/admin/broadcast/v2/stingers?sessionId=${r.id}`}
