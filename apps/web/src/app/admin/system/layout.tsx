...
/**
 * UI Audit Slice 4 (2026-04-28) — System hub layout.
 *
 * Trash + audit log + db health. The catch-all "platform plumbing"
 * surface for sysadmins. Today only Trash exists; audit log + db health
 * are scaffolded for future hubs.
 */

export const dynamic = "force-dynamic";

const HUB = ADMIN_HUBS.find((h) => h.key === "system")!;

export default async function SystemLayout({ children }: { children: ReactNode }) {
  const userClient = await getServerSupabase();
  const { data: auth } = await userClient.auth.getUser();
  if (!auth.user) redirect("/login?next=/admin/system");
  ...
  const { data: pub } = await userClient
    ...
    .maybeSingle();
  if (!pub) redirect("/login?next=/admin/system");
  ...
  return (
    <div className="space-y-6" data-testid="system-shell">
      <SectionHeader
        eyebrow="System"
        title="Platform plumbing"
        description="Trash recovery, audit log, and database health for the Elite 2025-26 platform."
      />
      <Breadcrumbs />
