...
async function resolveAdmin() {
  ...
  if (!auth.user) redirect("/login?next=/admin/branding");
  const { data: pub } = await userClient
    .from("users")
    .select("id")
    .eq("supabase_auth_id", auth.user.id)
    .maybeSingle();
  if (!pub) redirect("/login?next=/admin/branding");
  ...
  return (
    <div className="space-y-8">
      ...
      <nav className="flex gap-1 border-b border-[var(--ink-4)]">
        <TabLink href="/admin/branding?tab=logos" label="Logos" active={activeTab === "logos"} />
        <TabLink
          href="/admin/branding?tab=colors"
          label="Colors & Fonts"
