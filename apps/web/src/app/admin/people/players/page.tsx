...
async function resolveAdmin() {
  ...
  if (!auth.user) redirect("/login?next=/admin/people/players");
  const { data: pub } = await userClient
    .from("users")
    .select("id")
    .eq("supabase_auth_id", auth.user.id)
    .maybeSingle();
  if (!pub) redirect("/login?next=/admin/people/players");
...
export default async function AdminPlayersPage() {
  ...
  const cols: DataTableColumn<PlayerRow>[] = [
    ...
    },
    {
      ...
      render: (p) => (
        <Link
          href={`/admin/people/players/${p.id}/edit`}
  ...
  return (
    <div className="space-y-8">
      <SectionHeader
        ...
        action={
          <Link href="/admin/people/users/new">
      ...
      <DataTable
        ...
        emptyHint="Create users via /admin/people/users/new; the players table is seeded on first import."
      />
    </div>
  );
}
