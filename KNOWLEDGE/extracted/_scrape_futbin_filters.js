...
async function upsertRows(sb, rows, stats, inserted) {
  for (const r of rows) {
    ...
    else if (/rttf|road-to/.test(vLower)) itemType = "rttf";
    else if (r.variant && !/^(gold|silver|bronze|common|rare|normal|5_gold|4_silver|3_bronze|if)$/.test(vLower)) itemType = "special";
    if (exist) {
