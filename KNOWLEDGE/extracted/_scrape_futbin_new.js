  ...
  return page.evaluate(() => {
    ...
    for (const row of rows) {
      ...
      out.push({
        ...
        // nation icons have no alt/title attribute, so human-readable
        // nation names are resolved via the id→name table in
...
async function main() {
  ...
  try {
    ...
    console.log("[new] chem-rules refresh:", JSON.stringify(result));
  } catch (err) {
    console.error("[new] chem-rules refresh failed:", err && err.message ? err.message : err);
  }
  await ctx.close();
}
